#!/usr/bin/env node
/*********************************************************************************************************************
 *  Auspost Whitelist parser                                                                                          *
 *                                                                                                                    *
 *  This is an extension of the WAS Security Automations (SO0006-CloudFront)                                          *
 *  Using a S3 bucket and filename in Cloudformation as source for creating whitelist entries for AWS WAF             *
 *  The source code is copied from the reputation list parser and modified slightly to use,                           *
 *  a S3 bucket instead of https.get                                                                                  *
 *                                                                                                                    *
 *********************************************************************************************************************/
...
/**
 * Constructs a new object containing an URL to a whitelist list
 * @class
 * @classdesc An IP whitelist List
 * @param {string} url - URL to the whitelist list
 ...
 */
List.prototype.getRanges = function (callback, event) {
    var list = this;
    var ranges = [];
    var url = {
        Bucket: event.lists[0].bucketname,
        Key: event.lists[1].keyname
    };

    s3.getObject(url, function(err, data) { // Checking if the bucket and file exist
        // Handle any error and exit
        if (err)
            return err;
        var reader = readline.createInterface({ terminal: false, input: s3.getObject(url).createReadStream() }); // Using a S3 getObject readstream instead of https.get
